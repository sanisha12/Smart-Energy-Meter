...
#include <DNSServer.h> /* Local DNS Server used for redirecting all requests to
the configuration portal */
#include <ESP8266WebServer.h> /* Local WebServer used to serve the configuration
portal */
...
String apiKey = "GBH1K3293KFNO8WY"; /* Replace with your thingspeak API
key */
...
APPENDIX A
display.setTextColor(WHITE);
Dept. Of CSE, Vemana IT 53 2022-2023
display.print("Kwh");
display.display();
}
void ConnectAP(void) {
...
// Subtract min from max
  result = (((maxValue - minValue) * REF_VOLT) / RESOLUTION) *
  VOLTAGE_DIVIDER ;
  return result;
}
